    ...
    def serial_handle(self):
        ...
        if(bled_ready):
            ...
            for i in range(0,48):
                ...
                if(self.result[i] > base_intensity[i]+base_intensity[i]*20/100 or self.result[i] < base_intensity[i]-base_intensity[i]*20/100):
                    result_label[i]['bg'] = RESULT_LABEL_ERROR_BGD_COLOR
                    self.err = 1

            # Check system error 2
            if(average_current_intensity > average_base_intensity + average_base_intensity*20/100 or
                average_current_intensity < average_base_intensity - average_base_intensity*20/100):
                    for i in range(0,48):
                        result_label[i]['bg'] = RESULT_LABEL_ERROR_BGD_COLOR
                    self.err = 2
...
class QualitativeAnalysisFrame3(Frame):
    ...
    def serial_handle(self):
        ...
        if(bled_ready):
            ...
            positive_label.grid(row=1, column=0, padx=20, pady=10)
            positive_text_label = Label(self.annotate_result_frame, bg=MAIN_FUNCTION_FRAME_BGD_COLOR, text="POSITIVE (P)", height=2)
            positive_text_label.grid(row=1, column=1, padx=20, pady=10)
            na_label = Label(self.annotate_result_frame, bg=NA_COLOR, width=4, height=2)
            na_label.grid(row=2, column=0, padx=20, pady=10)
            na_copy_text_label = Label(self.annotate_result_frame, bg=MAIN_FUNCTION_FRAME_BGD_COLOR, text="NO SAMPLE (N/A)", height=2)
            na_copy_text_label.grid(row=2, column=1, padx=20, pady=10)
            
            # Report tab
            self.report_frame = ScrollableFrame1(report_tab)
            ...
            for i in range(0,48):
                if(self.base_window.qualitative_analysis_2.id_list[i] != 'N/A'):
                    ...
                    if(float(result_label[i]['text']) < self.base_window.qualitative_analysis_1.program_base_value * NUM_1):
                        sample_button_list[i]['text'] = self.base_window.qualitative_analysis_2.id_list[i]
                        result_button_list[i]['text'] = 'N'
                        result_button_list[i]['bg'] = NEGATIVE_COLOR
                        
                    # ~ elif(float(result_label[i]['text']) < self.base_window.qualitative_analysis_1.program_base_value * NUM_2):
                        # ~ sample_button_list[i]['text'] = self.base_window.qualitative_analysis_2.id_list[i] 
                        # ~ result_button_list[i]['text'] = 'P_L'
                        # ~ result_button_list[i]['bg'] = LOW_COPY_COLOR
                    else:
                        sample_button_list[i]['text'] = self.base_window.qualitative_analysis_2.id_list[i]
                        result_button_list[i]['text'] = 'P'
            ...
            # Save result file
            # ~ if(self.base_window.server_setting.server_state  == 1):
                # ~ wb = loaf_workbook(id_path + '/' + server_templete_file, keep_vba = True)
                # ~ sheet = wb.active
            # ~ else:
            wb = Workbook()
            sheet = wb.active
            ...
            wb.save(self.base_window.qualitative_analysis_1.analysis_result_folder + '/' + self.base_window.qualitative_analysis_2.id_file_name_label['text'] + '.xlsm')
...
class QuantitativeAnalysisFrame3(QualitativeAnalysisFrame3):
    ...
    def serial_handle(self):
        ...
        if(bled_ready):
            ...
            sheet.print_area = 'A1:G70'
            wb.save(self.base_window.quantitative_analysis_1.analysis_result_folder + '/' + self.base_window.quantitative_analysis_2.id_file_name_label['text'] + '.xlsm')
            ...
            msg = messagebox.showinfo("","COMPLETED")
            self.base_window.update_idletasks()
            subprocess.call(["scrot", self.base_window.quantitative_analysis_1.analysis_result_folder + "/result_capture.jpg"])
...
class ServerSettingFrame(Frame):
    def __init__(self, container):
        ...
        ip_label = Label(self.work_frame, bg=LABEL_BGD_COLOR, text="FTP Server :")
        ...
        path_label = Label(self.work_frame, bg=LABEL_BGD_COLOR, text="Path :")
        path_label.grid(row=4, column=0, sticky=E, pady=15, padx=30)
        
        self.ip_entry = Entry(self.work_frame, width=30, justify='right', font=('Courier',14))
        ...
        self.path_entry = Entry(self.work_frame, width=30, justify='right', font=('Courier',14))
    ...
    def connect_clicked(self):
        if(self.connect_button['text']=='Connect'):
            self.ip_set = self.ip_entry.get()
            self.user_set = self.user_entry.get()
            self.password_set = self.pass_entry.get()
            self.path_set = self.path_entry.get()
            if(self.ip_set==''):
                messagebox.showwarning("","Please enter yours server address.")
            elif(self.user_set==''):
                ...
                try:
                    ...
                    messagebox.showinfo("", "You are already connected to the server.")
                except Exception as e :
    ...
    def check_status(self):
        ...
        self.server_active = int(fr.readline())
        self.ip_set = fr.readline()
        self.user_set = fr.readline()
        self.password_set = fr.readline()
        self.path_set = fr.readline()
        fr.close()
        
        if(self.server_active == 0):
            self.connect_button['text'] = 'Connect'
            self.login_label['text'] = 'FTP'
            ...
            self.connnect_button['text'] = 'Disconnect'
